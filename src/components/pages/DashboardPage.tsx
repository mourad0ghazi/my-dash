...
export function DashboardPage() {
  ...
  return <motion.div className="dashboard-page" variants={staggerContainer} initial="hidden" animate="visible">
    <motion.section className="welcome-hero" variants={fadeInUp}>
      ...
      <div className="welcome-copy"><div className="eyebrow"><Sparkles size={14} /> {t('clarity')}</div><h1>{greeting}, {profile.name.split(' ')[0]} <span>✦</span></h1><p>{dateLabel(format(now, 'yyyy-MM-dd'), 'EEEE d MMMM yyyy', settings.language, settings.dateFormat)} · {t('essentials')}</p></div>
      <div className="hero-stats"><article><span className="hero-stat-icon"><WalletCards size={18} /></span><div><small>{t('balance')}</small><strong>{currency(balance, settings.currency, settings.language, settings.hideAmounts, settings.decimalSeparator)}</strong></div><em>{en ? '+12.4%' : '+12,4 %'}</em></article><article><span className="hero-stat-icon"><LayoutGrid size={18} /></span><div><small>{t('activeTasks')}</small><strong>{activeTasks}</strong></div><em>{tasks.length - activeTasks} {t('finished')}</em></article><article><span className="hero-stat-icon"><Flame size={18} /></span><div><small>{t('bestStreak')}</small><strong>{bestStreak} {t('days')}</strong></div><em>{en ? 'Meditation' : 'Méditation'}</em></article></div>
