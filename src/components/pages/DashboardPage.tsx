...
export function DashboardPage() {
  ...
  const layouts = useLifeStore(state => state.layouts); const visible = useLifeStore(state => state.visibleWidgets); const editMode = useLifeStore(state => state.editMode); const setEditMode = useLifeStore(state => state.setEditMode); const setLayouts = useLifeStore(state => state.setLayouts)
  ...
  const visibleIds = Object.keys(widgets).filter(id => visible[id]); const responsiveLayouts = useMemo(() => layouts as Layouts, [layouts])
  return <motion.div className="dashboard-page" variants={staggerContainer} initial="hidden" animate="visible">
    <motion.section className="welcome-hero" variants={fadeInUp}>
      ...
      <div className="hero-weather"><CloudSun size={26} /><span><strong>24°</strong><small>Casablanca · {t('weatherSunny')}</small></span></div>
    </motion.section>
    <div className="dashboard-toolbar"><div><h2>{t('yourDashboard')}</h2><p>{visibleIds.length} {t('modulesVisible')} · {t('deviceOnly')}</p></div><Button variant={editMode ? 'primary' : 'secondary'} onClick={() => setEditMode(!editMode)}><Pencil size={15} /> {editMode ? t('finish') : t('customize')}</Button></div>
