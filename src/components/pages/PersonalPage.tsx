...
export function PersonalPage() {
  ...
  return <motion.div className="inner-page" variants={pageTransition} initial="hidden" animate="visible" exit="exit">
    <header className="page-heading personal-heading"><div><span className="page-kicker"><Heart size={15} /> {t('personalKicker')}</span><h1>{t('personalTitle')}</h1><p>{t('personalSubtitle')}</p></div><div className="personal-score"><span>{t('balanceIndex')}</span><strong>84</strong><small>/100 · {t('goodRhythm')}</small></div></header>
    <div className="metric-strip personal-metrics"><article><small>{t('completedTasks')}</small><strong>{done}/{tasks.length}</strong><span>{t('thisWeek')}</span></article><article><small>{t('habitsToday')}</small><strong>{completedToday}/{habits.length}</strong><span>{t('keepGoing')}</span></article><article><small>{t('activeGoals')}</small><strong>{goals.length}</strong><span>{Math.round(goals.reduce((sum, goal) => sum + goal.progress / goal.target * 100, 0) / Math.max(1, goals.length))}% {t('average')}</span></article></div>
