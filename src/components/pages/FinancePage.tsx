...
export function FinancePage() {
  ...
  return <motion.div className="inner-page" variants={pageTransition} initial="hidden" animate="visible" exit="exit">
    <header className="page-heading"><div><span className="page-kicker"><PiggyBank size={15} /> {t('financeKicker')}</span><h1>{t('financeTitle')}</h1><p>{t('financeSubtitle')}</p></div><div className="heading-balance"><small>{t('trackedWealth')}</small><strong>{currency(86800, settings.currency, settings.language, settings.hideAmounts)}</strong><span>↗ {en ? '+6.3%' : '+6,3 %'} {t('thisYear')}</span></div></header>
