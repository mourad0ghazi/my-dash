...
export function FinancePage() {
  ...
  const savings = useLifeStore(state => state.savings)
  const investments = useLifeStore(state => state.investments)
  ...
  const trackedWealth = savings.reduce((sum, goal) => sum + goal.current, 0) + investments.reduce((sum, investment) => sum + investment.value, 0)
  const savingsRate = income > 0 ? Math.round((income - expense) / income * 100) : 0
  return <motion.div className="inner-page" variants={pageTransition} initial="hidden" animate="visible" exit="exit">
    <header className="page-heading"><div><span className="page-kicker"><PiggyBank size={15} /> {t('financeKicker')}</span><h1>{t('financeTitle')}</h1><p>{t('financeSubtitle')}</p></div><div className="heading-balance"><small>{t('trackedWealth')}</small><strong>{currency(trackedWealth, settings.currency, settings.language, settings.hideAmounts, settings.decimalSeparator)}</strong><span>{savings.length + investments.length} {en ? 'tracked assets' : 'actifs suivis'}</span></div></header>
