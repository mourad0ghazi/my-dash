...
import { Badge, Button, EmptyState, IconButton, RingProgress, Select } from '../ui/primitives'
...
export function WeatherWidget() {
  ...
  const [weather, setWeather] = useState<{ temp: number; apparent: number; humidity: number; wind: number; code: number; daily: number[] } | null>(null); const [loading, setLoading] = useState(false)
  useEffect(() => {
    const city = cities[settings.weatherCity]
    if (!city) { setWeather(null); setLoading(false); return }
    ...
    fetch(`https://api.open-meteo.com/v1/forecast?latitude=${city.lat}&longitude=${city.lon}&current=temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m,weather_code&daily=temperature_2m_max&timezone=auto&forecast_days=3`, { signal: controller.signal })
      .then(response => { if (!response.ok) throw new Error('weather'); return response.json() }).then(data => setWeather({ temp: Math.round(data.current.temperature_2m), apparent: Math.round(data.current.apparent_temperature), humidity: data.current.relative_humidity_2m, wind: Math.round(data.current.wind_speed_10m), code: data.current.weather_code, daily: data.daily.temperature_2m_max.map(Math.round) })).catch(error => { if (error instanceof Error && error.name !== 'AbortError') setWeather(null) }).finally(() => setLoading(false))
    return () => controller.abort()
  }, [settings.weatherCity])
  const locale = language === 'en' ? 'en-US' : 'fr-FR'
  const selector = <Select aria-label={l('Ville météo', 'Weather city')} className="select-mini" value={settings.weatherCity} onChange={event => update({ weatherCity: event.target.value })}><option value="" disabled>{l('Choisir…', 'Choose…')}</option>{Object.keys(cities).map(city => <option value={city} key={city}>{city}</option>)}</Select>
  if (!settings.weatherCity) return <Widget id="weather" title={l('Météo', 'Weather')} icon={CloudSun} action={selector}><EmptyState icon={<CloudSun size={22} />} title={l('Ville à choisir', 'Choose a city')} text={l('Sélectionnez votre ville pour charger une météo réelle.', 'Select your city to load real weather data.')} /></Widget>
  if (!weather) return <Widget id="weather" title={l('Météo', 'Weather')} icon={CloudSun} action={selector}><EmptyState icon={<Cloud size={22} />} title={loading ? l('Chargement…', 'Loading…') : l('Météo indisponible', 'Weather unavailable')} text={loading ? l('Connexion au service météo.', 'Connecting to the weather service.') : l('Réessayez plus tard ou choisissez une autre ville.', 'Try again later or choose another city.')} /></Widget>
  const { label, Icon } = weatherMeta(weather.code, language === 'en')
  return <Widget id="weather" title={l('Météo', 'Weather')} icon={CloudSun} action={selector}>
...
export function PomodoroWidget() {
  const { l } = useCopy()
  const modes = { focus: 25 * 60, short: 5 * 60, long: 15 * 60 }; const [mode, setMode] = useState<keyof typeof modes>('focus'); const [seconds, setSeconds] = useState(modes.focus); const [running, setRunning] = useState(false); const [sessions, setSessions] = useState(0)
