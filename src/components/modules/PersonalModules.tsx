...
import { dateLabel, relativeDate, todayISO } from '../../utils/formatters'
...
export function ClockWidget() {
  const [date, setDate] = useState(new Date()); const [analog, setAnalog] = useState(false); const settings = useLifeStore(state => state.settings); const { l, locale } = useModuleCopy()
  ...
  return <Widget id="clock" title={l('Horloge', 'Clock')} icon={AlarmClock} action={<button className="mini-switch" onClick={() => setAnalog(value => !value)}>{analog ? l('Numérique', 'Digital') : l('Analogique', 'Analog')}</button>}>
    <div className="clock-body">{analog ? <AnalogClock date={date} /> : <><div className="digital-time">{time}</div><p>{format(date, 'EEEE d MMMM', { locale })}</p><small>{l('Semaine', 'Week')} {format(date, 'II')} · {settings.timezone.split('/').pop()}</small></>}</div>
  </Widget>
}
...
export function TasksWidget({ expanded = false }: { expanded?: boolean }) {
  ...
  const tasks = useLifeStore(state => state.tasks); const setStatus = useLifeStore(state => state.setTaskStatus); const add = useLifeStore(state => state.addTask); const remove = useLifeStore(state => state.deleteTask); const update = useLifeStore(state => state.updateTask)
...
export function CalendarWidget({ expanded = false }: { expanded?: boolean }) {
  ...
  const weekLabels = language === 'en' ? ['M', 'T', 'W', 'T', 'F', 'S', 'S'] : ['L', 'M', 'M', 'J', 'V', 'S', 'D']
