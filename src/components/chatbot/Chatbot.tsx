...
function getReply(input: string) {
  ...
  if (/budget|left|reste|combien.*mois/.test(q)) {
    ...
    const warning = [...state.budgets].filter(item => item.planned > 0).sort((a, b) => b.spent / b.planned - a.spent / a.planned)[0]
    if (!warning || planned <= 0) return en ? 'No budget has been entered yet. Add one category and its monthly amount, then I can help you review it.' : 'Aucun budget n’est encore renseigné. Ajoutez une catégorie et son montant mensuel, puis je pourrai vous aider à l’analyser.'
    return en
  ...
  if (/epargne|economis|fonds|saving|emergency fund/.test(q)) {
    ...
    if (!goal || goal.target <= 0) return en ? 'You have not created a savings goal yet. Give it a name, an amount and a date to get a useful pace.' : 'Vous n’avez pas encore créé d’objectif d’épargne. Donnez-lui un nom, un montant et une date pour obtenir un rythme utile.'
  ...
  if (/objectif|progression|but|goal|progress/.test(q)) return state.goals.length ? state.goals.map(goal => `• ${localizeSavingsTitle(goal.title, state.settings.language)}: ${goal.target > 0 ? Math.round(goal.progress / goal.target * 100) : 0}%`).join('\n') : (en ? 'You have not created a goal yet. Add one first, then I can track its progress with you.' : 'Vous n’avez pas encore créé d’objectif. Ajoutez-en un, puis je pourrai suivre sa progression avec vous.')
  ...
  if (/meteo|weather|pluie|rain/.test(q)) return state.settings.weatherCity ? (en ? `Open the Weather card for current data and forecasts in ${state.settings.weatherCity}.` : `Consultez la carte Météo pour les données actualisées et les prévisions à ${state.settings.weatherCity}.`) : (en ? 'Add your city in Settings to display local weather.' : 'Ajoutez votre ville dans les Paramètres pour afficher la météo locale.')
...
export function Chatbot() {
  ...
  const [input, setInput] = useState(''); const bottomRef = useRef<HTMLDivElement>(null); const firstName = useLifeStore(state => state.profile.name.trim().split(/\s+/)[0] || ''); const en = settings.language === 'en'
  useEffect(() => bottomRef.current?.scrollIntoView({ behavior: 'smooth' }), [messages, typing, open])
  useEffect(() => { if (!settings.coachEnabled || settings.coachFrequency === 'never') return; const key = 'lifeos:last-coach'; const last = localStorage.getItem(key); const today = new Date().toISOString().slice(0, 10); if (last !== today) { const id = window.setTimeout(() => { push({ role: 'assistant', text: `${firstName ? `${firstName}, ` : ''}${en ? 'your quick overview is ready' : 'votre mini-bilan est prêt'}: ${getReply('coach bilan')}` }); if (!open) setUnread(1); localStorage.setItem(key, today) }, 1800); return () => window.clearTimeout(id) } }, [])
