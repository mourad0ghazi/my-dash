...
export default function App() {
  const [page, setPage] = useState<PageId>(getPage); const [booting, setBooting] = useState(() => !useLifeStore.persist.hasHydrated()); const [locked, setLocked] = useState(true); const [pin, setPin] = useState(''); const [pinError, setPinError] = useState(false); const [assistantReady, setAssistantReady] = useState(false)
  ...
  useEffect(() => { const onHash = () => setPage(getPage()); window.addEventListener('hashchange', onHash); if (!window.location.hash) window.location.hash = '#/'; const hydrated = useLifeStore.persist.onFinishHydration(() => setBooting(false)); if (useLifeStore.persist.hasHydrated()) setBooting(false); const bootFallback = window.setTimeout(() => setBooting(false), 1500); const onLock = () => setLocked(true); const onPageShow = (event: PageTransitionEvent) => { if (event.persisted) setLocked(true) }; const onVisibilityChange = () => { if (document.visibilityState === 'hidden') setLocked(true) }; window.addEventListener('lifeos:lock', onLock); window.addEventListener('pageshow', onPageShow); document.addEventListener('visibilitychange', onVisibilityChange); const serviceWorkerTimer = window.setTimeout(() => { if (!('serviceWorker' in navigator)) return; if (import.meta.env.PROD) navigator.serviceWorker.register('sw.js').catch(() => undefined); else navigator.serviceWorker.getRegistrations().then(items => Promise.all(items.filter(item => [item.active, item.waiting, item.installing].some(worker => worker?.scriptURL.endsWith('/sw.js'))).map(item => item.unregister()))).catch(() => undefined) }, 2500); return () => { window.removeEventListener('hashchange', onHash); window.removeEventListener('lifeos:lock', onLock); window.removeEventListener('pageshow', onPageShow); document.removeEventListener('visibilitychange', onVisibilityChange); hydrated(); window.clearTimeout(bootFallback); window.clearTimeout(serviceWorkerTimer) } }, [])
  useEffect(() => {
    if (locked) return
  ...
  const unlock = () => {
    if (pin === ACCESS_PIN) { setLocked(false); setPin(''); setPinError(false) }
  ...
  if (locked) return <div className="lock-screen lock-screen-in"><form className="lock-card" onSubmit={event => { event.preventDefault(); unlock() }}><span className="lock-logo"><LockKeyhole size={28} /></span><img src="assets/logo.svg" alt="LifeOS" /><h1>{en ? 'Welcome back' : 'Heureux de vous revoir'}</h1><p>{en ? 'Enter your password to access your LifeOS space.' : 'Saisissez votre mot de passe pour accéder à votre espace LifeOS.'}</p><Input id="lifeos-access-pin" autoFocus autoComplete="current-password" type="password" inputMode="numeric" maxLength={4} value={pin} onChange={event => { if (/^\d{0,4}$/.test(event.target.value)) setPin(event.target.value); setPinError(false) }} aria-label={en ? '4-digit password' : 'Mot de passe à 4 chiffres'} placeholder="••••" className={pinError ? 'input-error pin-input' : 'pin-input'} />{pinError && <small className="error-text">{en ? 'Incorrect password. Try again.' : 'Mot de passe incorrect. Réessayez.'}</small>}<Button type="submit">{en ? 'Enter LifeOS' : 'Entrer dans LifeOS'}</Button><span className="privacy-line"><ShieldCheck size={14} /> {en ? 'Access protected on this device' : 'Accès protégé sur cet appareil'}</span></form></div>
